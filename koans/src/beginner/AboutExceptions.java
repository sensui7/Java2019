...
public class AboutExceptions {
    ...
    @Koan
    public void useFinally() { // finally is good for recovering resources after a try block
        String s = "";
        try {
            doStuff();
            s += "code ran normally";
    ...
    private void tryCatchFinallyWithVoidReturn(StringBuilder whatHappened) {
        try {
            ...
            whatHappened.append(", but so did the finally!"); // will execute even if there's a return above
        }
    }
    ...
    public void passArgumentValidationWithAnIllegalArgumentException() {
        ...
        assertEquals(s, "5"); // should be a string instead
    }
