...
public class AboutStreams {
    ...
    public void filteredCount() {
        long count = places.stream()
                .filter(s -> s.startsWith("S")) // using lambdas too
                .count();
    ...
    public void reduceWithoutStarterReturnsOptional() {
        Optional<String> join = places.stream()
                .reduce(String::concat);
        assertEquals(join.get(), "BelgradeZagrebSarajevoSkopjeLjubljanaPodgorica"); //interpreted the same "almost like unboxing?"
    }
    ...
    public void rangeToList() {
        ...
        Integer[] l = {1, 2, 3};
        assertEquals(range, Arrays.asList(l));
    }
}
